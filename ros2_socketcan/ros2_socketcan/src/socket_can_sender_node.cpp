...
LNI::CallbackReturn SocketCanSenderNode::on_configure(const lc::State & state)
{
  ...
  std::string to_can_bus_topic = this->declare_parameter<std::string>(
    "to_can_bus_topic", "to_can_bus");

  if (!enable_fd_) {
    frames_sub_ = this->create_subscription<can_msgs::msg::Frame>(
      to_can_bus_topic, 500, std::bind(&SocketCanSenderNode::on_frame, this, std::placeholders::_1));
  } else {
    fd_frames_sub_ = this->create_subscription<ros2_socketcan_msgs::msg::FdFrame>(
      to_can_bus_topic, 500, std::bind(
