...
LNI::CallbackReturn SocketCanReceiverNode::on_configure(const lc::State & state)
{
  ...
  std::string from_can_bus_topic = this->declare_parameter<std::string>(
    "from_can_bus_topic", "from_can_bus");

  if (!enable_fd_) {
    frames_pub_ = this->create_publisher<can_msgs::msg::Frame>(from_can_bus_topic, 500);
  } else {
    fd_frames_pub_ =
      this->create_publisher<ros2_socketcan_msgs::msg::FdFrame>(from_can_bus_topic, 500);
  }
